...
int ExecuteSQL(const char *sql, const char *outputFileName) {
  ...
  const char *alpahUnder = "_abcdefghijklmnopqrstuvwxzABCDEFGHIJKLMNOPQRSTUVWXY"
                           "Z"; // 全てのアルファベットの大文字小文字とアンダーバーです。
  const char *
      alpahNumUnder = "_abcdefghijklmnopqrstuvwxzABCDEFGHIJKLMNOPQRSTUVWXYZ0123"
  ...
  const Token keywordConditions[] = {
      {AND, "AND"},   {ASC, "ASC"},       {BY, "BY"},
      {DESC, "DESC"}, {FROM, "FROM"},     {ORDER, "ORDER"},
      {OR, "OR"},     {SELECT, "SELECT"}, {WHERE, "WHERE"}};
  ...
  for (size_t i = 0; i < sizeof(tokens) / sizeof(tokens[0]); i++) {
    tokens[i].kind = NOT_TOKEN;
    strncpy(tokens[i].word, "", MAX_WORD_LENGTH);
  }
  ...
  while (*charactorCursol) {
    ...
    if (*search) {
      Token literal = (Token){.kind = INT_LITERAL,
    ...
    if (*search) {
      Token identifier = (Token){.kind = IDENTIFIER,
                                 .word = ""}; // 読み込んだ識別子の情報です。
  ...
  for (size_t i = 0;
       i < sizeof(whereExtensionNodes) / sizeof(whereExtensionNodes[0]); i++) {
  ...
  if (tokenCursol->kind == ASTERISK) {
    ...
    while (tokenCursol->kind == COMMA || first) {
      ...
      if (tokenCursol->kind == IDENTIFIER) {
        ...
        strncpy(selectColumns[selectColumnsNum].columnName, tokenCursol->word,
                MAX_WORD_LENGTH);
        ++tokenCursol;
        if (tokenCursol->kind == DOT) {
          ...
          if (tokenCursol->kind == IDENTIFIER) {
            ...
            strncpy(selectColumns[selectColumnsNum].columnName,
                    tokenCursol->word, MAX_WORD_LENGTH);
            ++tokenCursol;
          } else {
            error = ERR_SQL_SYNTAX;
            goto ERROR;
    ...
    if (tokenCursol->kind == WHERE) {
      ...
      while (true) {
        ...
        if (tokenCursol->kind == PLUS || tokenCursol->kind == MINUS) {
          ...
          if (tokenCursol[1].kind != IDENTIFIER &&
              tokenCursol[1].kind != INT_LITERAL) {
  ...
  for (size_t i = 0; i < sizeof(inputColumns) / sizeof(inputColumns[0]); i++) {
    for (size_t j = 0; j < sizeof(inputColumns[0]) / sizeof(inputColumns[0][0]);
         j++) {
  ...
  Column
      allInputColumns[MAX_TABLE_COUNT *
  ...
  while (true) {
    ...
    Data **row = outputData[outputRowsNum] = malloc(
        MAX_COLUMN_COUNT * sizeof(Data *)); // 出力している一行分のデータです。
    if (!row) {
      error = ERR_MEMORY_ALLOCATE;
      goto ERROR;
    }
      ...
      for (int j = i + 1; j < outputRowsNum; ++j) {
        ...
        for (int k = 0; k < orderByColumnIndexesNum; ++k) {
          Data *mData = allColumnOutputData
              [minIndex]
              [orderByColumnIndexes[k]]; // インデックスがminIndexのデータです。
